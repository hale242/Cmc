...
{
    use AuthorizesRequests, DispatchesJobs, ValidatesRequests, GF;
    ...
    {
        $res = DB::table('user')->where('user_id', session('session_admin_id'))->first();
        return $res;
    }
    ...
    {
        $result = SlideBanner::orderby('sort', 'asc')->get();
        return $result;
    }
    ...
    public static function NextDate($start, $next, $type)
    {
        ...
        switch ($type) {
            case "D":return (date("Y-m-d", mktime(0, 0, 0, $strdate["1"], $strdate["2"] + $next, $strdate["0"])));
                break;
            case "M":return (date("Y-m-d", mktime(0, 0, 0, $strdate["1"] + $next, $strdate["2"], $strdate["0"])));
                break;
            case "Y":return (date("Y-m-d", mktime(0, 0, 0, $strdate["1"], $strdate["2"], $strdate["0"] + $next)));
                break;
