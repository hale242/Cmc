...
<script src="{{asset('assets/admin/js/fancybox/jquery.fancybox.js')}}"></script>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(".select2_category_data").select2({
            ajax: {
                url: "{{url('api/admin/category/getdata')}}",
            ...
            success: function(data) {
                ...
                if (data.result == true) {
                    swal.fire({
                        ...
                        if (data.type == 'add') {
                            location.href = "{{url('admin/course/main')}}";
                        } else {
                            location.reload();
                        }
                    });
...
<script type="text/javascript">
    $(function() {
        ...
        $(document).on('click', '.btn_del_question', function() {
            ...
            var url_del = "{{url('admin/course/deletequestion/')}}";
            var url_div = "{{url('admin/course/loadquestion/')}}" + "/" + test_id;
            var token = $('input[name=_token]').val();
        ...
        $(document).on('click', '.btn_edit_question', function() {
            ...
            var url_que = "{{url('admin/course/loadquestionedit/')}}" + "/" + question_id;
            var url_div = "{{url('admin/course/loadchoice/')}}" + "/" + question_id;
            var url_img = "{{url('upload/member/choice/')}}";
            var url_queimg = "{{url('upload/member/question/')}}";
        ...
        $(document).on('click', '.delete_question_img', function() {
            ...
            var url_del = "{{url('admin/course/deleteimage/')}}";
            var token = $('input[name=_token]').val();
        ...
        $(document).on('click', '.btn-remove-ans-edit', function() {
            ...
            var url_del = "{{url('admin/course/deletechoice/')}}";
            var url_div = "{{url('admin/course/loadquestion/')}}" + "/" + test_id;
            var token = $('input[name=_token]').val();
        ...
        function SaveQuestion() {
            ...
            var checker = $("input[name='chkanswer_t[]']").map(function() {
                return $(this).prop("checked");
            }).get();
            var url_api = "{{url('api/admin/course/addquestion')}}";
            var url_div = "{{url('admin/course/loadquestion/')}}" + "/" + test_id;
            ...
            var choice_id = $("input[name='answer[]']").map(function() {
                return $(this).attr('data-id');
            }).get();
            var choice_img = $("input[name='choi_img[]']").map(function() {
                return $(this).val();
            }).get();
...
<script type="text/javascript">
    ...
    function btn_create_lesson(type_action, lesson_file, course_id, lesson_id, lesson_name, vdo_url, detail, status) {
        ...
        if (type_action == 'Create') {
            ...
            var url_div = "{{url('admin/course/loadlession/')}}" + "/" + lesson_id;
            $('#lesson_file').empty();
            $.get(url_div, function(data) {
                $.each(data, function(k, v) {
                    ...
                    $("#lesson_file").append('<div class="col-md-4">' +
                        '<a target="_blank" data-fancybox="gallery_lession' + v['lesson_id'] + '" style="border: 1px solid #ccc;padding: 5px;margin-right: -8px;margin-bottom: 5px;" href="{{asset('
                        upload / member / lesson / file / ')}}/' + v['lesson_file'] + '">' + namefile + '</a>&nbsp;&nbsp;' +
            ...
            var url_div = "{{url('admin/course/loadcontentlession/')}}" + "/" + lesson_id;
            $.get(url_div, function(data) {
        ...
        $(document).on('click', '.btn-delete-file', function() {
            ...
            var url_del = "{{url('admin/course/delete/LessonFileDelete/')}}" + "/" + id;
...
<script type="text/javascript">
    $(document).on('click', '.SubMenu', function() {
        var id = $(this).attr('id');
        $('#Table_Submenu').html('');
        $('#add_sudmenu_id').val(id);
        $.ajax({
            url: "{{url('admin/menusystem/submenu/')}}" + "/" + id,
            success: function(data) {
                var res = data.content;
                var item_submenu = '';
                var item_submenu_name = '';
                var status = '';
                var number = 1;
                // console.log(res);
                if (res.length > 0) {
                    $.each(res, function(k, v) {
                        item_submenu_name = '';
                        $.each(v.menu_system_lang, function(k, v1) {
                            item_submenu_name += v1.menu_system_lang_name+'<br>';
                        });
                            if (v.menu_system_status == 1) {
                                status = '<span class="label label-success">Activate</span>';
                            } else {
                                status = '<span class="label label-danger">Inctivate</span>';
                            }
                            item_submenu += '<tr>\
                        <td align="center"><label for="example-text-input">' + number + '</label></td>\
                        <td><label for="example-text-input"></label>'+item_submenu_name+'</td>\
                        <td><label for="example-text-input">' + v.menu_system_path + '</label></td>\
                        <td><label for="example-text-input">' + v.menu_system_icon + '</label></td>\
                        <td align="center"><label for="example-text-input">' + v.menu_system_z_index + '</label></td>\
                        <td align="center"><label for="example-text-input">' + status + '</label></td>\
                        <td align="center"><label for="example-text-input"><button type="button" class="btn btn-success EditSubMenu" id="' + v.menu_system_id + '" ><i class="fa fa-edit"></i></button></label></td>\
                        </tr>';
                            number++;
                    });
                    ...
                    item_submenu += '<tr>\
                        <td colspan=7 align="center"><label for="example-text-input" class="col-form-label" >Not Data</label></td>\
                        </tr>';
                }
                $('#Table_Submenu').append(item_submenu);
...
<script type="text/javascript">
    $(document).on('click', '.AddSubMenu', function() {
        var id = $('#add_sudmenu_id').val();
        $('#menu_system_main_menu_id').val(id);
        $('#modal_CreateSubMenu').modal('show');
...
<script type="text/javascript">
    $(document).on('click', '.EditSubMenu', function() {
        ...
        $.ajax({
            url: "{{url('admin/menusystem/getdatasubmenu/')}}" + "/" + id,
            success: function(data) {
                var res = data.content;
                $('#edit_menu_system_path').val(res.menu_system_path);
                $('#edit_menu_system_icon').val(res.menu_system_icon);
                $('#edit_menu_system_z_index').val(res.menu_system_z_index);

                $.each(res.menu_system_lang, function(k, v) {
                    $('#edit_menu_system_lang_code_' + v.language_id).val(v.menu_system_lang_code);
                    $('#edit_menu_system_lang_name_' + v.language_id).val(v.menu_system_lang_name);
                    $('#edit_menu_system_lang_details_' + v.language_id).val(v.menu_system_lang_details);
                });
            }
        })
    })
