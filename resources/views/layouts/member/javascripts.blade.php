...
<script src="{{asset('assets/member/js/custom.js?t'.date('YmdHis'))}}"></script>
...
$('#subscribe_frm').on('submit', function(event){
    ...
    $.post('{{url('subscribe')}}', $( "#subscribe_frm" ).serialize(), function(data) {
        if(data==1){
            $('.alert_sub').slideDown();
...
$('#subscribe_frm_mobile').on('submit', function(event){
    ...
    $.post('{{url('subscribe')}}', $( "#subscribe_frm_mobile" ).serialize(), function(data) {
        if(data==1){
